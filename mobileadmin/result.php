...
require_once "Db.php";
    ...
    $objProduct = new Db();
    $total = count($objProduct->find_name($_GET['key']));
    $total_links = ceil($total/$per_page);
    $show = $objProduct->limitProductKey($url,$per_page,$page,$_GET['key']); // show[] all san pham
    ?>
				...
				<div class="span12">
					<div class="widget-box">
						...
						<div class="widget-content nopadding">
							...
							<ul class="pagination">
								<?php echo  $objProduct->firstOrLastPage($url,1,"<<",$page,$total_links,$key).$objProduct->preBtn($page,$url,$key).$objProduct->paginate($url,$total,$page,$per_page,2,$key).$objProduct->nextBtn($page,$url,$total_links,$key).$objProduct->firstOrLastPage($url,$total_links,">>",$page,$total_links,$key); ?>
							</ul>
						</div>
					</div>
				</div>
